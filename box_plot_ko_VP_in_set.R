a <- read.table("DESeq2_EZH2KO_WT_enrich_padj005_protcod_WTVP_ind_EZH2_enriched_all_ratios_KO_VP_ind.txt", sep = "\t", stringsAsFactor = FALSE, header = TRUE)
...
library(ggplot2)
s <- p
write.csv(s, "Box_plot_EZH2_miR_repressed_genes_KO_VP_ind_set.csv")
pdf("Box_plot_EZH2_miR_repressed_genes_KO_VP_ind_set.pdf")
p <- ggplot(s, aes(s$id, s$log_norm_exp, fill = id))
p + geom_boxplot(outlier.colour = "white", notch = TRUE) + theme_bw() + theme(axis.text.x=element_text(angle=60, hjust=1), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + coord_cartesian(ylim = c(0,20)) + scale_fill_manual(values = c("#E69F00", "#56B4E9")) + geom_line(aes(group = s$X), size = 0.05, alpha = 0.7, colour = "dark grey")
dev.off()
