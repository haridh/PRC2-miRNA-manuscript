...
library(ggplot2)
s <- p
write.csv(s, "Box_plot_EZH2_miR_repressed_genes.csv")
pdf("Box_plot_EZH2_miR_repressed_genes.pdf")
p <- ggplot(s, aes(s$id, s$log_norm_exp, fill = id))
p + geom_boxplot(outlier.colour = "white", notch = TRUE) + theme_bw() + theme(axis.text.x=element_text(angle=60, hjust=1), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + stat_boxplot(geom ='errorbar', width = 0.2) + coord_cartesian(ylim = c(0,20)) 
dev.off()
